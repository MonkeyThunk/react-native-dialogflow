...
dialogflow.setConfiguration = function(accessToken, languageTag) {
  ...
  dialogflow.sessionId = dialogflow.sessionId
...
dialogflow2.setConfiguration = async function(
  clientEmail,
  privateKey,
  languageTag,
  projectId
) {
  dialogflow2.accessToken = await dialogflow2.generateAccessToken(
    clientEmail,
    privateKey
  );
  dialogflow2.languageTag = languageTag;
  dialogflow2.projectId = projectId;
  dialogflow2.sessionId = dialogflow2.sessionId
    ? dialogflow2.sessionId
    : dialogflow2.guid();
  ...
  Voice.onSpeechEnd = c => {
    if (dialogflow2.speechResult) {
      dialogflow2.requestQuery(
        dialogflow2.speechResult[0],
        dialogflow2.onResult,
        dialogflow2.onError
      );
    }
  ...
  Voice.onSpeechResults = result => {
    if (result.value) {
      ...
      //HEALTHJAY
      dialogflow2.onSpeechResult(result.value[0]);
    }
  };
};
